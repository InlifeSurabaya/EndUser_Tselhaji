<div class="flex min-h-screen bg-neutral-50 text-neutral-800">
    ...
    <div class="hidden lg:flex lg:w-1/2 flex-col justify-center items-center p-12 text-white bg-gradient-to-br from-primary-600 to-highlight-700">
        ...
        <h1 class="text-3xl font-bold text-center mb-2">Bergabung dengan Telkomsel</h1>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
        <div class="w-full max-w-md">
            ...
            <div class="text-center mb-10 lg:hidden">
                ...
                <p class="text-neutral-600 mt-2">Buat akun Telkomsel Anda.</p>
            </div>
            ...
            <form wire:submit="register" class="mt-8 space-y-5">
                ...
                <div>
                    ...
                    <input wire:model.live="name" id="name" type="text" placeholder="Contoh: lorem ipsum" required
                        class="py-3 px-4 block w-full border border-neutral-300 rounded-lg text-sm placeholder:text-neutral-400 focus:border-primary-500 disabled:opacity-50 disabled:pointer-events-none transition @error('name') !border-error @enderror">
                    @error('name') <p class="text-sm text-error mt-2">{{ $message }}</p> @enderror
                </div>
                ...
                {{-- Input Nomer Telkomsel --}}
                <div>
                    <label for="notelkom" class="block text-sm font-medium text-neutral-700 mb-2">No Telkomsel</label>
                    <input wire:model.live="notelkom" id="notelkom" type="number" placeholder="Contoh: 081xxx" required
                        class="py-3 px-4 block w-full border border-neutral-300 rounded-lg text-sm placeholder:text-neutral-400 focus:border-primary-500 disabled:opacity-50 disabled:pointer-events-none transition @error('notelkom') !border-error @enderror">
                    @error('notelkom') <p class="text-sm text-error mt-2">{{ $message }}</p> @enderror
                </div>

                {{-- Input No Whatsapp --}}
                <div
                    x-data="{ sama: false }"
                    x-init="$watch('sama', value => {
                        if (value) $watch(() => $wire.notelkom, val => $wire.set('nowa', val))
                    })">
                    <div class="flex items-center justify-between mb-2">
                        <label for="nowa" class="block text-sm font-medium text-neutral-700">
                            No WhatsApp
                        </label>
                        ...
                        <button
                            ...
                            @click="
                                sama = !sama;
                                if (sama) {
                                    $wire.set('nowa', $wire.notelkom);
                                } else {
                                    $wire.set('nowa', '');
                                }
                            ">
                            <span x-text="sama ? 'Batalkan' : 'Samakan dengan No Telkomsel'"></span>
                        </button>
                    </div>

                    <input
                        wire:model.live="nowa"
                        id="nowa"
                        type="number"
                        placeholder="Contoh: 081xxx"
                        required
                        x-bind:disabled="sama"
                        class="py-3 px-4 block w-full border border-neutral-300 rounded-lg text-sm 
                        placeholder:text-neutral-400 focus:border-primary-500 
                        disabled:bg-neutral-100 disabled:cursor-not-allowed 
                        transition @error('nowa') !border-error @enderror">
