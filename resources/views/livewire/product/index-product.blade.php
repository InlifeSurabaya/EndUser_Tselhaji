                        ...
                        @if ($product->discount > 0)
                            ...
                            <div class="flex justify-center items-center gap-2">
                                <span
                                    ...
                                    {{ Number::currency($product->harga, 'IDR', 'id') }}
                                </span>
                                <span
                            ...
                            <p class="text-2xl font-bold text-primary-600 mt-1">
                                {{ Number::currency($product->harga - ($product->harga * $product->discount) / 100, 'IDR', 'id') }}
                            </p>
                        @else
                    ...
                    <div class="p-4 overflow-y-auto max-h-[60vh]">
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-semibold text-neutral-800">{{ $selectedProduct->country->name ?? 'Global' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-neutral-600">Masa Aktif:</span>
                                <span class="font-semibold text-neutral-800">{{ $selectedProduct->validity_days }} Hari</span>
                            </div>
                                ...
                                @if ($selectedProduct->discount > 0)
                                    <div class="flex justify-center items-center gap-2">
                                        <span class="text-xl font-medium text-neutral-400 line-through">
                                            {{ Number::currency($selectedProduct->harga, 'IDR', 'id') }}
                                        </span>
                                        <span class="inline-flex items-center gap-x-1 py-1 px-2 rounded-md text-xs font-medium bg-red-100 text-red-800">
                                            {{ $selectedProduct->discount }}% OFF
                                        </span>
                                    </div>
                                    <p class="text-3xl font-bold text-primary-600 mt-1">
                                        {{ Number::currency($selectedProduct->harga - ($selectedProduct->harga * $selectedProduct->discount) / 100, 'IDR', 'id') }}
                                    </p>
                                @else
